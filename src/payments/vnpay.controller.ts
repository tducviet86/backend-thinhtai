...
class CreateVnpayPaymentDto {
  ...
  @IsIn(['vn', 'en']) locale: 'vn' | 'en' = 'vn';
  @IsOptional() @IsIn(['VNPAYQR', 'VNBANK', 'INTCARD']) bankCode?: 'VNPAYQR' | 'VNBANK' | 'INTCARD';
}
@Controller('payments/vnpay')
export class VnpayController {
  constructor(private readonly service: VnpayService) {}
  @Post('create') @Version('1') create(@Body() dto: CreateVnpayPaymentDto, @Req() req: Request) { return this.service.createPaymentUrl(req.user!.sub, dto.bookingCode, req.ip || req.socket.remoteAddress || '127.0.0.1', dto.locale, dto.bankCode); }
