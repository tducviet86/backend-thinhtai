...
export class BookingsService {
  ...
  async create(dto: CreateBookingDto, userId: string) {
    ...
    if (!customer) throw new ForbiddenException('Customer profile must be completed before booking');
    ...
    if (!supplied || supplied.expiresAt <= new Date() || supplied.unitId !== unitId || supplied.checkIn.valueOf() !== start.valueOf() || supplied.checkOut.valueOf() !== end.valueOf() || supplied.guests !== dto.guestCount || supplied.total.toFixed(2) !== calculated.total) throw new ConflictException('Quote is expired or booking details have changed');
    return this.prisma.$transaction(async (tx) => { await this.availability.lockUnit(tx, unitId); await tx.hold.updateMany({ where: { unitId, status: 'ACTIVE', expiresAt: { lte: new Date() } }, data: { status: 'EXPIRED' } }); await this.availability.assertAvailable(tx, unitId, start, end); const fees = new Prisma.Decimal(calculated.cleaningFee).add(calculated.serviceFee); return tx.booking.create({ data: { bookingCode: `TT-${new Date().getUTCFullYear()}-${randomBytes(8).toString('hex').toUpperCase()}`, customerId: customer.id, unitId, source: dto.source, checkIn: start, checkOut: end, guestCount: dto.guestCount, status: 'PENDING_PAYMENT', currency: calculated.currency, subtotal: calculated.subtotal, fees, discount: calculated.discount, total: calculated.total, normalTotal: calculated.total, depositRequired: calculated.requiredDeposit, remainingAmount: calculated.total, createdByUserId: userId }, select: { id: true, bookingCode: true, status: true, checkIn: true, checkOut: true, total: true, depositRequired: true, remainingAmount: true, currency: true } }); }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
  }
  async findPublic(code: string) { const b = await this.prisma.booking.findUnique({ where: { bookingCode: code }, select: { bookingCode: true, status: true, checkIn: true, checkOut: true, guestCount: true, currency: true, total: true, depositRequired: true, paidAmount: true, remainingAmount: true, unit: { select: { publicCode: true, nameVi: true, nameEn: true } } } }); if (!b) throw new NotFoundException('Booking not found'); return b; }
}
