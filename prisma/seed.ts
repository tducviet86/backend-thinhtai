...
async function main():Promise<void>{
  ...
  const property=await prisma.property.upsert({where:{slugVi:'panoma'},create:{name:'ABC Apartment',slugVi:'panoma',slugEn:'panoma',descriptionVi:'Căn hộ dịch vụ hiện đại tại Đà Nẵng.',descriptionEn:'Modern serviced apartments in Da Nang.',locationId:city.id,address:'Mỹ An, Ngũ Hành Sơn, Đà Nẵng',latitude:16.0471,longitude:108.2445,checkInTime:'14:00',checkOutTime:'11:00',status:'ACTIVE',publishedAt:new Date()},update:{name:'ABC Apartment',descriptionVi:'Căn hộ dịch vụ hiện đại tại Đà Nẵng.',descriptionEn:'Modern serviced apartments in Da Nang.',status:'ACTIVE'}});
  ...
  for(const [index,image] of panoramaImages.entries())mediaItems.push(await prisma.media.upsert({where:{storageKey:image.storageKey},create:{...image,mimeType:'image/jpeg',fileSize:0,altVi:`Ảnh panorama căn hộ ABC Apartment ${index+1}`,altEn:`ABC Apartment panorama ${index+1}`},update:{url:image.url,width:image.width,height:image.height,mimeType:'image/jpeg',altVi:`Ảnh panorama căn hộ ABC Apartment ${index+1}`,altEn:`ABC Apartment panorama ${index+1}`}}));
